...
class RSAService:
    def __init__(self):
        ...
        self.rc5 = RC5(self.md5_service.hexdigest("Default key").encode('utf-8'))

        self.rsa = RSA()
        self.private_key, self.public_key = self.rsa.generate_key()
        self.encrypted_message = "There is no encrypted message for now. Use the menu."
        self.decrypted_message = "There is no decrypted message for now. Use the menu."
